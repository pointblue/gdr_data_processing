# Script for pre-processing light-level geolocation data from 1718
...
bucketlist()
tmp = tmp = get_bucket_df(bucket = "deju-penguinscience", prefix="AllData/gdr_datashare/GDR_1718/") # 84 files

## Get info on retrieved devices in "Z:/Informatics/S031/analyses/GDR/data/croz_royds_gdr_depl_all.csv"
depl_dat <- read.csv("data/croz_royds_gdr_depl_all_v2021-08-27.csv")%>%
  mutate(bird_id=as.numeric(bird_id))%>%
  subset(season=="2017"&retrieve_date!="NA")%>%
  mutate(id=str_pad(gsub("NA","",bird_id),9,pad="0"))
filelist<-depl_dat$filename # 88 files
...
drift <- read.csv("data/clock_drift_1718.csv") # 68 complete data files, including 50945 (rebooted)
filelist<-drift$filename # 68 complete files

## Load the raw data for interactive twilight annotation
index=68:length(filelist)
for(i in index){
  # pull out filename and paste to create path
  fn<-filelist[i]
  print(paste("Loading", fn, "...", sep=" "))
  path=paste0("s3://deju-penguinscience/AllData/gdr_datashare/GDR_1718/",fn, ".txt",sep="")
  ...
  print(paste(drift$cdrift_av[drift$filename==filelist[i]], "min"))
  cdrift <- drift$cdrift_av[drift$filename==filelist[i]]
  end <- raw$Date[length(raw$Date)]+cdrift*60
  ...
  deploy_date <- as.POSIXct(depl_dat$deploy_date[which(depl_dat$filename==paste0(filelist[i], ".txt"))], format="%Y-%m-%d", tz="GMT")
  retrieve_date <- as.POSIXct(depl_dat$retrieve_date[which(depl_dat$filename==paste0(filelist[i], ".txt"))], format="%Y-%m-%d", tz="GMT")
  raw <- subset(raw, raw$Date>=deploy_date & raw$Date<=retrieve_date)
  #Release coordinates
  if (depl_dat$br_col[which(depl_dat$filename==paste0(filelist[i], ".txt"))]=="CROZ") {
  ...
  id <- fn
  write.csv(twl_filt, paste0("data/geoloc/1718/twl/", id,
  ...
  write.csv(twl, paste0("data/geoloc/1718/twl/", fn,
...
drift <- read.csv("data/clock_drift_1718.csv") # 68 complete data files, including 50945 (rebooted)
filelist_c <- paste0(as.character(drift$filename), ".txt") # 68 files
depl_dat <- read.csv("data/croz_royds_gdr_depl_all.csv")%>%
  mutate(bird_id=as.numeric(bird_id))%>%
  subset(season=="2017"&retrieve_date!="NA")%>%
  mutate(id=str_pad(gsub("NA","",bird_id),9,pad="0"))
filelist_t<-as.character(depl_dat$filename[!is.na(depl_dat$filename)]) # 83 files
filelist_i <- c(setdiff(filelist_c, filelist_t), setdiff(filelist_t, filelist_c))

# Load the raw data for interactive twilight annotation
index=15:length(filelist_i)
for (i in index){
  # pull out filename and paste to create path
  fn<-filelist_i[i]
  print(paste("Loading", fn, "...", sep=" "))
  path=paste0("s3://deju-penguinscience/AllData/gdr_datashare/GDR_1718/", fn, sep="")
  raw <-s3read_using(fread, object = path, skip=25, drop=6, col.names=c("date","time","temp","pressure","lux"))
  raw$Date  <- as.POSIXct(paste(raw$date, raw$time), format = "%d/%m/%Y %H:%M:%S", tz = "GMT")
  raw$Light  <- log(ifelse(raw$lux<1, 1, raw$lux))
  head(raw)
  ...
  print(head(twl))
  id <- substr(fn,1,nchar(fn)-4)
  write.csv(twl, paste0("data/geoloc/1718/twl/", id,
                        "_twl.csv"), row.names = F)
  #Additional automated cleaning/filtering 
  #twl <- twl[!twl$Deleted,]
  #twl_filt <- twilightEdit(twilights = twl,
