...
source("code/functions/setupSession_1718.R")

tmpDir <- tempdir()

fls <- drive_ls("geoloc/1718/twl/", n_max = 500)
...
fitFls <- data.frame(path = as.character(list.files("~/Google Drive/Shared/geoloc/results_1718/fit/", pattern = "*.RData", full.names=TRUE)))
fitFls$ids <- as.numeric(sapply(strsplit(list.files("~/Google Drive/Shared/geoloc/results_1718/fit/", pattern = "*.RData"), "_"), function(x) as.character(x[[1]])))
fitFls$dts <- as.POSIXct(Sys.Date()) - file.info(as.character(fitFls$path))$ctime 
...
for(ind in 1:nrow(fls)) {
    ...
    drive_upload(paste0(tmpDir, "/", mTab$id[mdatInd], "_maps_iter1.pdf"), 
                 path = paste0("geoloc/results_1718/preRun/", mTab$id[mdatInd], "_maps_iter1.pdf"), overwrite = T, verbose = F)
    unlink(paste0(tmpDir, mTab$id[mdatInd], "_maps_iter1.pdf"))
      ...
      drive_upload(paste0(tmpDir, "/fit.RData"), 
                   path = paste0("geoloc/results_1718/fit/", mTab$id[mdatInd], "_fit.RData"), overwrite = T, verbose = F)
      ...
      drive_upload(paste0(tmpDir, "/movSum.csv"), 
                   path = paste0("geoloc/results_1718/movementSummary/", mTab$id[mdatInd], "_movementSummary.csv"), overwrite = T, verbose = F)
      ...
      drive_upload(paste0(tmpDir, "/track.png"), 
                   path = paste0("geoloc/results_1718/movementSummary/trackMaps/", mTab$id[mdatInd], "_track.csv"), overwrite = T, verbose = F)
