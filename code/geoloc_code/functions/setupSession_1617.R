...
##### get metadata
mdat    <- drive_get("croz_royds_1617_retr_withdd.csv")
tmpPath <- tempdir()
drive_download(mdat$path, paste0(tmpPath, "/metadata.csv"), overwrite = T, verbose = F)

mTab    <- read.csv(paste0(tmpPath, "/metadata.csv"), sep = ";")
...
# mTab$id <- as.numeric(lapply(strsplit(sapply(strsplit(as.character(mTab$filename), "_"), function(x) x[[2]]), ".txt"), function(y) y[[1]]))
unlink(tmpPath)
