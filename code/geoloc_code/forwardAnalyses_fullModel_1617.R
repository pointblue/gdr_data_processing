...
source("code/functions/setupSession_1617.R")

tmpDir <- tempdir()

mTab <- subset(mTab, !is.na(final_run) & !final_run)

fls <- drive_ls("geoloc/1617/twl/", n_max = 500)
  fls$ind_id <- sapply(strsplit(fls$name, "_"), function(x) as.character(x[[2]]))
...
for(ind in 1:nrow(fls)) {
    ...
    if(is.na(mTab$best_zenith)[mdatInd]) {
    ...
    drive_upload(paste0(tmpDir, "/", mTab$id[mdatInd], "_maps_iter1.pdf"), 
                 path = paste0("geoloc/results_1617/preRun/", mTab$id[mdatInd], "_maps_iter1.pdf"), overwrite = T, verbose = F)
    unlink(paste0(tmpDir, mTab$id[mdatInd], "_maps_iter1.pdf"))
      ...
      drive_upload(paste0(tmpDir, "/fit.RData"), 
                   path = paste0("geoloc/results_1617/fit/", mTab$id[mdatInd], "_fit.RData"), overwrite = T, verbose = F)
      ...
      drive_upload(paste0(tmpDir, "/movSum.csv"), 
                   path = paste0("geoloc/results_1617/movementSummary/", mTab$id[mdatInd], "_movementSummary.csv"), overwrite = T, verbose = F)
      ...
      drive_upload(paste0(tmpDir, "/track.png"), 
                   path = paste0("geoloc/results_1617/movementSummary/trackMaps/", mTab$id[mdatInd], "_track.csv"), overwrite = T, verbose = F)
